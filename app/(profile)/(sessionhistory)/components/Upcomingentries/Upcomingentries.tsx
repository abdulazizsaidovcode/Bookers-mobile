...
const Upcomingentries = () => {
  ...
  const renderItem = ({ item }: any) => (
    <View style={tw`bg-gray-700 p-4 rounded-lg mb-4`}>
      ...
      <Text style={tw`text-gray-400 mb-4`}>{item.orderDate}</Text>
      <View style={tw`flex-row justify-between mb-4`}>
        <Text style={tw`text-gray-400`}>{item.startTime}</Text>
        <Text style={tw`text-gray-400`}>{item.finishTime}</Text>
      </View>
      <View style={tw`flex-row justify-between`}>
        ...
        <TouchableOpacity
          style={tw`bg-gray-500 p-2 rounded-lg flex-row items-center`}
        >
